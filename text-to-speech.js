...
class TextToSpeech {
  ...
  readFiles(){
    ...
    fs.readdir(testFolder, (err, files) => {
      files.forEach(file => {
        ...
        let currentFileObj = {"path":filePath,"name":fileName,"ext":fileExt,"content":contents};
  ...
  convertFiles(){
    var self = this;
    let newFilePath = this.exportFolder + this.txtArray[this.currentFile].name + this.exportExt;
    ...
    let request = https.get("https://text-to-speech-demo.ng.bluemix.net/api/v1/synthesize?text="+this.txtArray[this.currentFile].content+"&voice=en-US_LisaV3Voice&ssmlLabel=SSML&download=true&accept=audio%2Fmp3",
    function(response) {
      console.log(`done`);
